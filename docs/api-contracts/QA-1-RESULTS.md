...
**Phase 8 result:** all matrix cells PASS (self-approval INTENTIONAL/OBS-G, day-counting a CAL-1 NOTE — neither a FAIL).

## Phase 9 — Library

**Fix landed:** OBS-E-4 (`issue.service.returnBook` `returned_at` + overdue day-count UTC-local → `todayAdInNepal()` + TZ-independent diff; 2 files + mocked-clock test). **Live-proven off-by-one** (returned_at was 2026-07-12, now 2026-07-13). See QA-1-BUGS.md.

| Module | Feature | C | R | U | D | Staff | Student | State/scoping | Status |
|---|---|---|---|---|---|---|---|---|---|
| Library | Book category / book / copies | PASS | PASS | PASS | (soft) | PASS | catalog read (ALL_ROLES) | — | PASS |
| Library | Member register | PASS (LIB-2083-0001) | PASS | PASS | — | PASS | — | — | PASS |
| Library | **Issue (inventory decrement)** | PASS (copy `available true→false`) | PASS | — | — | PASS | — | BS due `2083-03-21` | PASS |
| Library | Double-issue | 400 "not available" + psql no-change | — | — | — | PASS | — | — | PASS |
| Library | **Return (inventory restore + fine)** | — | — | PASS (copy `false→true`; RETURNED; **returned_at 2026-07-13 Nepal**; fine_days 8; **fine_amount 80.00 NUMERIC(8,2)**) | — | PASS | — | — | PASS |
| Library | Double-return | 404 "already returned" + psql no-change | — | — | — | PASS | — | — | PASS |
| Library | Member scoping | — | — | — | — | — | catalog 200; **issues → 403**; POST issues → 403 | staff-only issue surface | PASS |

**Phase 9 result:** all cells PASS. **Inventory round-trip psql-proven** (issue decrements available copy, return restores it); double-issue → 400 + double-return → 404, both with psql no-change; **OBS-E-4 live-proven** (returned_at = Nepal-today 2026-07-13, not the old UTC-local 2026-07-12); overdue fine day-count Nepal-date-correct (8 days), `fine_amount` **NUMERIC(8,2)** (2-decimal money; smaller precision than finance's 10,2 but adequate for fines). BS due-date displayed server-side (`2026-07-05 → 2083-03-21`). Member scoping: catalog open to all, **issue surface staff-only → student 403** (no cross-student view exists, so no IDOR possible).

- **BUG-3 scope note:** `fine_amount = fine_days × fine_per_day` in `returnBook` is JS-float (8×10=80.00 exact here) — same class as the finance JS-float money, tracked under **BUG-3 → MON-1** (Prisma.Decimal). Not fixed (per decision).

## Phase 10 — Dashboard & Cross-Cutting

**Fixes landed:** OBS-E-5 (dashboard UTC-today ×2 + weekly window → Nepal; 2 files + 2 tests) and OBS-C (`getStats.byStatus` real enum; API + web + test). **🔴 One CRITICAL FAIL — BUG-4 (cross-tenant leak), STOP-and-report.**

| Area | Check | Result | Status |
|---|---|---|---|
| Dashboard | overview stats vs psql | `students.total 5 = psql 5`, `active 5 = psql 5`, `asOf 2026-07-13` | PASS |
| Dashboard | weekly-attendance (OBS-E-5) | 7 days, **`weekEnd 2026-07-13`** (Nepal-today) | PASS |
| Dashboard | role scoping | student `/dashboard/overview` → **403** | PASS |
| Theming | `GET /tenants/verify/:slug` | 200 `{primaryColor:"#2563EB", primaryForeground, logoUrl}` | PASS |
| **OBS-C** | PASSED_OUT vs stats.byStatus | student→PASSED_OUT → `byStatus.PASSED_OUT=1` (5-status enum) | **FIXED** |
| Push reg | per-user + upsert not dup | register 201 (count 1) → re-register 201 (count **1**), per-user, ANDROID; DELETE 204; **zero sends** | PASS |
| Base64 census | report-only | 4 legacy base64 blobs: logos (jorden-donovan, motherland-school) + motherland student & staff photo; qa-demo clean (FILE-1 keys). Migration = separate session. | REPORT |
| Isolation (data) | demo owner → qa_demo `:id` | `GET /students/:qa_demo_S1` → **404** (not in demo schema) | PASS |
| **Isolation (token)** | qa_demo token + `X-Tenant-Slug: demo` | was **200 leak** → **now 403** after the BUG-4 fix (`TenantMatchGuard`) | **FIXED → PASS** |

**Phase 10 result:** all cells PASS after the BUG-4 fix. **OBS-C, OBS-E-5, and BUG-4 all fixed** (BUG-4 = global `TenantMatchGuard`; closing proofs: leak→403, cross-tenant WRITE→403 + psql no-write, same-tenant 200, file/report/dashboard covered, data-level 404, public login intact — see QA-1-BUGS.md). Dashboard stats match psql; theming responds; push registration upserts (no dup, zero sends); base64 census report-only (4 legacy blobs). Staff CRUD + soft-delete (OBS-E-3 `end_date`=Nepal-today); full leave loop with `reviewed_by` actor accountability; **state-machine integrity enforced** (reject-after-approve + double-approve → 400, no state change); **non-approver → 403**. Two flags per architect decisions: **OBS-G** self-approval permitted (product ruling, not fixed), **CAL-1** leave = calendar days (no Saturday/holiday exclusion, no fix). BS-month bucket boundary correct (a cross-Gregorian-month BS month splits on the right AD day); fee-aging `asOf` is Nepal-today (`todayAdInNepal()`); all report endpoints 200 for admin tier with correct aggregates; **teacher/student/parent → 403** on every report endpoint (requirement 3). No UTC-date-truncation in the report/bucketing code (requirement 2 — grep clean). Marks entry stamps `entered_by`=teacher; **out-of-range marks rejected 400 with psql proof of no write**; full pipeline marks→result(grade/rank)→report-card verified (Aarav 85%→A→rank 1); **publish privacy gate proven** (parent/student see only published terms — `examResults` empty until publish; staff see all); report-card JSON + **PDF valid** (`%PDF-` magic, 23964 B non-zero, download round-trip) for student `/me` and parent own-child; **cross-family report-card + PDF → 403**.
