...
const SUBMISSION_STYLE: Record<SubmissionStatus, string> = {
  SUBMITTED: 'bg-green-50 text-green-700 dark:bg-green-500/10 dark:text-green-400',
...
export default function AssignmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <Link href="/assignments" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-brand-500">
          ...
          <div className="flex items-center gap-2">
            ...
            {assignment.status === 'PUBLISHED' && (
              <Button variant="outline" onClick={handleClose} disabled={close.isPending}>
                {close.isPending ? <Loader2 className="mr-1.5 h-4 w-4 animate-spin" /> : <Lock className="mr-1.5 h-4 w-4" />}
                Close
              </Button>
            )}
          </div>
      ...
      <div className="rounded-sm border border-stroke bg-white p-5 shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="flex flex-wrap gap-x-8 gap-y-2 text-sm">
          <span className="text-gray-500">Due: <BsDate date={assignment.dueDate} /></span>
          {assignment.publishedAt && (
            <span className="text-gray-500">Published: <BsDate date={assignment.publishedAt} /></span>
          )}
        </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {assignment.attachmentKeys.map((key, i) => (
              <FileDownloadLink
                key={key}
                value={key}
                className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-3 py-1 text-xs text-brand-500 hover:underline dark:bg-meta-4"
              >
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-2 text-left dark:bg-meta-4">
              <tr>
                {['Roll', 'Student', 'Submitted', 'Status', 'Marks', 'Answer / File', ''].map((h) => (
                  <th key={h} className="px-4 py-2.5 font-medium text-black dark:text-white">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-stroke dark:divide-strokedark">
              {view.submissions.map((s) => (
                <tr key={s.id}>
                  <td className="px-4 py-2.5 font-mono text-gray-500">{s.rollNumber ?? '—'}</td>
                  <td className="px-4 py-2.5 font-medium text-black dark:text-white">{s.studentName}</td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      {s.textAnswer && (
                        <span className="max-w-52 truncate text-gray-500" title={s.textAnswer}>{s.textAnswer}</span>
                      )}
                      {s.fileKey && (
                        <FileDownloadLink value={s.fileKey} className="text-brand-500 hover:underline text-xs">
          ...
          <div className="border-t border-stroke px-5 py-4 dark:border-strokedark">
            ...
            {view.missing.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {view.missing.map((m) => (
                  <span key={m.studentId} className="rounded-full bg-red-50 px-3 py-1 text-xs text-red-600 dark:bg-red-500/10 dark:text-red-400">
    ...
    <Dialog open={!!submission} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md">
        ...
        {submission?.textAnswer && (
          <div className="max-h-40 overflow-y-auto rounded-md bg-gray-50 p-3 text-sm text-gray-600 dark:bg-meta-4 dark:text-gray-300">
            {submission.textAnswer}
          </div>
        )}
        <div className="space-y-4">
