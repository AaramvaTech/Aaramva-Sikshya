-- 0001_baseline.sql — frozen baseline of the tenant schema (MIG-1).
-- The runner substitutes {{schema}} with the tenant schema name (e.g. tenant_sxs)
-- and applies the whole file inside a single transaction.
-- This is migration 0001; all 12 pre-existing tenants bootstrap against it as a no-op.
-- Every statement is idempotent (CREATE ... IF NOT EXISTS / guarded ALTER) so it is
-- safe to (re-)apply against an already-up-to-date schema.

CREATE SCHEMA IF NOT EXISTS "{{schema}}";

-- LOCAL so it is scoped to the migration transaction (we run this through a
